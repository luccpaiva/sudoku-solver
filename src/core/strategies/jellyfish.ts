...
export function jellyfish(snapshot: Snapshot): StratResult | null {
  ...
  for (let digit = 1; digit <= 9; digit++) {
    ...
    for (let r = 0; r < 9; r++) {
      ...
      if (cells.length >= 2 && cells.length <= 4) rowMap.set(r, cells)
    }

    if (rowMap.size >= 4) {
      for (const rowCombo of combinations([...rowMap.keys()], 4)) {
        ...
        return buildResult(
          ...
          allCells,
          toEliminate,
          `Jellyfish on ${digit}: rows ${rowCombo.map(r => r + 1).join('/')} ` +
    ...
    for (let c = 0; c < 9; c++) {
      ...
      if (cells.length >= 2 && cells.length <= 4) colMap.set(c, cells)
    }

    if (colMap.size >= 4) {
      for (const colCombo of combinations([...colMap.keys()], 4)) {
        ...
        return buildResult(
          ...
          allCells,
          toEliminate,
          `Jellyfish on ${digit}: cols ${colCombo.map(c => c + 1).join('/')} ` +
